function solve() {
  ...
  const inputArr = document.querySelector("#inputs>textarea");

  function onClick() {
    let objRestaurants = [];

    for (let restaurant of JSON.parse(inputArr.value)) {
      ...
      objRestaurant.averageSalary =
        sumSalary / objRestaurant.employees.length || 0;
    }

    objRestaurants = objRestaurants.sort(
    ...
    let strRepresentBestRest = `Name: ${
      bestRestaurant.name
    } Average Salary: ${bestRestaurant.averageSalary.toFixed(
      2
    )} Best Salary: ${bestRestSalary.toFixed(2)}`;
    let workersBestRest = "";
    ...
    document.getElementById("bestRestaurant").querySelector("p").textContent =
      strRepresentBestRest;
    document.getElementById("workers").querySelector("p").textContent =
      workersBestRest;
  }
}
