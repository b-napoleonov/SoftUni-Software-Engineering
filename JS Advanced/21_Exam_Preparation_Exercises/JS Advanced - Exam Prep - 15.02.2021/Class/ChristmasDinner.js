class ChristmasDinner {
  ...
  inviteGuests(name, dish) {
    if (this.dishes.find((d) => d.recipeName == dish) == false) {
  ...
  showAttendance() {
    ...
    for (const name in this.guests) {
      let dishProducts = this.dishes.find((d) => d.recipeName == this.guests[name]).productsList;
      result.push(`${name} will eat ${this.guests[name]}, which consists of ${dishProducts.join(", ")}`);
    }
