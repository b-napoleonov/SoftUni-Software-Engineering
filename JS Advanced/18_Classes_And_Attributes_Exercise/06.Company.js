class Company{
    ...
    addEmployee(username, salary, position, department) {
        ...
        let args = [username, salary, position, department];
    ...
    bestDepartment() {
        ...
        if (bestDepartment != '') {
            this.departments[bestDepartment].sort((x, y) => y.salary - x.salary || x.username.localeCompare(y.username));
            ...
            for (const worker of Object.values(this.departments[bestDepartment])) {
                output += `${worker.username} ${worker.salary} ${worker.position}\n`;
            }
